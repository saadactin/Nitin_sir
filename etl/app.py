...
def index():
    ...
    server_conf = sqlservers.get("server1", {})
    server = server_conf.get("server", "localhost")
    username = server_conf.get("username", "sa")
    password = server_conf.get("password", "root")

    conn_str = (
        f"DRIVER={{ODBC Driver 18 for SQL Server}};"
        f"SERVER={server};UID={username};PWD={password};"
        f"Trusted_Connection=no;Encrypt=no;"
    )
    ...
    try:
        ...
        logging.error(f"Error fetching databases from {server}: {e}")
        databases = []

    return render_template("index.html", sqlservers=sqlservers, pg_conf=pg_conf, databases=databases)
